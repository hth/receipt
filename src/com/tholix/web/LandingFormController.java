...
public class LandingFormController {
	...
	public ModelAndView loadForm(@ModelAttribute("userSession") UserSession userSession, HttpSession session) {
		...
		UserProfileEntity userProfileEntity = userProfileManager.getObject(userSession.getProfileId());
		populate(userProfileEntity);
		
		ModelAndView modelAndView = new ModelAndView(nextPageIsCalledLanding);
		List<ReceiptEntity> receipts = receiptManager.getAllObjectsForUser(userSession.getProfileId());
	...
	public ModelAndView create(UploadReceiptImage uploadReceiptImage, BindingResult result, HttpSession session) {
		...
		try {
		    ...
		    ReceiptEntity receiptEntity = ReceiptEntity.newInstance(uploadReceiptImage.getDescription(), receiptBlobId, userSession.getProfileId());
